...
DOMSelectors.result.addEventListener("click", function () {
  ...
  DOMSelectors.box.insertAdjacentHTML(
    "beforeend",
    `<div id ="section"><h1 id="yourname">Hello ${info.name}</h1> <p id ="yourinfo">Your favorite book is ${info.book}. Your favorite author is ${info.author}. Your favorite genre is ${info.genre}</p><button type="button" id="erase${count}">Remove</button></div>`
  );
  DOMSelectors.name.value = "";
  DOMSelectors.book.value = "";
  DOMSelectors.author.value = "";
  DOMSelectors.genre.value = "";
  DOMSelectors[`erase${count}`] = document.getElementById(`erase${count}`);
  DOMSelectors[`erase${count}`].addEventListener("click", function () {
    document.getElementById("yourname").remove();
    document.getElementById("yourinfo").remove();
    document.getElementById(`erase${count}`).remove();
  });
});
